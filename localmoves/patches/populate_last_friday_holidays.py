"""
Patch: Populate Last Friday Holiday data
...
def execute():
    """Populate Last Friday Holiday data - runs once on migration"""
    
    # Data extracted from sheet - Last Friday of each month (excluding bank holidays)
    data = [
        # 2026
    ...
    for year, month_name, date_str in data:
        try:
            ...
            frappe.log_error(f"Error creating {month_name} {year}: {str(e)}", "Last Friday Holiday Patch")
    
    frappe.db.commit()
    
    print(f"✅ Last Friday Holiday Patch: Created {created_count}, Skipped {skipped_count}")
